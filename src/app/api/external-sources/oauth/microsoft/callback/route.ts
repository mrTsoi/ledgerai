...
export async function GET(req: Request) {
  try {
    ...
    if (!clientId || !clientSecret) {
      return sendResultHtml({ type: 'external_oauth', ok: false, error: 'Microsoft OAuth is not configured' })
    }
    ...
    if (!tokenRes.ok) {
      ...
      return sendResultHtml({ type: 'external_oauth', ok: false, error: tokenJson?.error_description || tokenJson?.error || 'Token exchange failed', details, hint: 'Verify MICROSOFT_OAUTH_CLIENT_ID/SECRET and that the exact redirect URI is configured in Azure App Registration. Expected redirect_uri: ' + redirectUri })
    }

    const refreshToken = tokenJson.refresh_token as string | undefined
    if (!refreshToken) {
      const details = process.env.NODE_ENV !== 'production' ? tokenJson : undefined
      return sendResultHtml({ type: 'external_oauth', ok: false, error: 'No refresh_token returned', details, hint: 'Ensure you requested offline_access and that the user granted consent. Try disconnecting and reconnecting.' })
    }
    ...
    if ((source as any).provider !== 'ONEDRIVE') {
      return sendResultHtml({ type: 'external_oauth', ok: false, error: 'Source is not OneDrive' })
    }
    ...
    try {
      console.info('[oauth/microsoft/callback] success, source_id=', String(parsed.source_id))
    } catch (e) {}
    ...
    console.error('Unhandled error in /api/external-sources/oauth/microsoft/callback', e)
