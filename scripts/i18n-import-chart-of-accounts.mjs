...
async function main() {
  ...
  let query = supabase.from('chart_of_accounts').select('name,account_type,account_subtype,description,is_active')
  ...
  for (const row of data || []) {
    ...
    if (row?.account_type) texts.push(row.account_type)
    if (row?.account_subtype) texts.push(row.account_subtype)
  ...
  console.log(`Found ${unique.length} unique chart_of_accounts strings (name/account_type/account_subtype/description).`)
